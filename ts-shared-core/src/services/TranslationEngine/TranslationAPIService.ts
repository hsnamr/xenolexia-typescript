...
// API response shapes (for typed response.json())
...
export class TranslationAPIService {
  ...
  constructor(customProviders?: TranslationAPIConfig[], keyValueStore?: IKeyValueStore) {
    this.providers = customProviders || [...DEFAULT_PROVIDERS];
    this.storage = keyValueStore ?? memoryKeyValueStore;
  }
  ...
  private async getLibreTranslateLanguages(): Promise<Language[]> {
    try {
      ...
      const data = (await response.json()) as LibreTranslateLanguagesResponse;
  ...
  private async getLingvaLanguages(): Promise<Language[]> {
    try {
      ...
      const data = (await response.json()) as LingvaLanguagesResponse;
  ...
  private async getFromCache(key: string): Promise<TranslationResult | null> {
    ...
    try {
      const stored = await this.storage.getItem(CACHE_PREFIX + key);
      if (stored) {
  ...
  private async saveToCache(key: string, result: TranslationResult): Promise<void> {
    ...
    try {
      await this.storage.setItem(CACHE_PREFIX + key, JSON.stringify(result));
    } catch (error) {
  ...
  async clearCache(): Promise<void> {
    ...
    try {
      ...
      if (this.storage.multiRemove) await this.storage.multiRemove(cacheKeys);
    } catch (error) {
...
export const translationAPI = new TranslationAPIService(undefined, memoryKeyValueStore);
