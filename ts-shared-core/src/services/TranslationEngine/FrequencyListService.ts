...
export class FrequencyListService {
  ...
  constructor(keyValueStore?: IKeyValueStore) {
  ...
  async clearCache(language?: Language): Promise<void> {
    if (language) {
      ...
      await this.storage.removeItem(STORAGE_PREFIX + language);
    } else {
      this.cache.clear();
  ...
  private async loadFromStorage(language: Language): Promise<FrequencyList | null> {
    try {
      const stored = await this.storage.getItem(STORAGE_PREFIX + language);
      if (stored) {
  ...
  private async saveToStorage(language: Language, list: FrequencyList): Promise<void> {
    try {
      await this.storage.setItem(STORAGE_PREFIX + language, JSON.stringify(list));
    } catch (error) {
