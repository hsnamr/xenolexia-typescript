...
export class ChapterContentService {
  ...
  async loadEpub(filePath: string): Promise<void> {
    try {
      ...
      if (this.fileSystem.readFileAsBase64) {
        base64 = await this.fileSystem.readFileAsBase64(filePath);
      } else {
  ...
  async getChapterHtml(
    ...
    // Word replacement: run TranslationEngine when options and factory are provided
    if (translationOptions && this.engineFactory) {
      try {
        const engine = this.engineFactory(translationOptions);
